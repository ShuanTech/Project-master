	...
	try{
		if(!file_exists('../photos')){
			...
			if(!file_exists('../photos/'.$fileName)){
				mkdir('../photos/'.$fileName,0777,true);
				mkdir('../photos/'.$fileName.'/pro_pic',0777,true);
			}else if(!file_exists('../photos/'.$fileName.'/pro_pic')){
				mkdir('../photos/'.$fileName.'/pro_pic',0777,true);
			}
		}
	}catch(Exception $e){}
	
	$path='../photos/'.$fileName.'/pro_pic/';
	$l_path='photos/'.$fileName.'/pro_pic/';
	$fName=date('ymdhi').'.jpg';
	$file_path = $path . $fName;
		if(move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $file_path)) 
		{
			$link="http://www.udyomitra.com/".$l_path.$fName;
			$sql="select pro_pic from login where u_id='".$fileName."'";
			$res=mysql_query($sql);
			$val=mysql_fetch_row($res);
			if($val[0]==null){
				$ins="update login set pro_pic='".$link."' where u_id='".$fileName."'";
				$res=mysql_query($ins);
				...
				if($val1[0]==null){
					$ins="insert into user_imgs(u_id,pro_pic) values('".$fileName."','".$val[0]."')";
					$res=mysql_query($ins);
					/* if($res>0){
					...
					$data=$val[0].','.$link;
					$upt="update user_imgs set pro_pic='".$data."' where u_id='".$fileName."'";
					$res=mysql_query($upt);
					/* if($res>0){
						$response['message']="Updated";
						$response['success']=1;
