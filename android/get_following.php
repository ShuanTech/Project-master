...
if(isset($_POST['u_id'])){
	...
	$data=array();
	$sql="SELECT l.u_id,l.full_name,l.pro_pic,l.level FROM `login` l,following f WHERE l.u_id=f.following 
		and f.u_id='".$_POST['u_id']."' and f.close=0 ORDER BY f.id DESC";
	...
	if(mysql_num_rows($result)>0){
			...
			while($row=mysql_fetch_array($result)){
				...
				array_push($response["following"],$data);
			}
				$response['success']=1;
